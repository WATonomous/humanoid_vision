...
class RosDataStreamer:
    def __init__(self, host='localhost', port=9090, topic_name='/data', msg_type='std_msgs/Float32MultiArray'):
    ...
    def publish(self, data):
        if isinstance(data, (np.ndarray, pd.DataFrame, pd.Series)):
            arr = np.asarray(data)
            msg = {'layout': {'dim': [], 'data_offset': 0}, 'data': arr.astype(np.float32).ravel().tolist()}
